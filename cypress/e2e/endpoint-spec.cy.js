describe('wikipedia spec', () => {
  ...
  const watchListUpperMenuSelector = '#pt-watchlist';
  ...
  it('add two articles to watchlist and remove one of them', () => {
    ...
    assertTextIsPresent('You have 1 page on your watchlist')
    searchOnWiki(secondArticle);
    addToWatchList(secondArticle);
    assertTextIsPresent('You have 2 pages on your watchlist')
  ...
  function searchOnWiki(text) {
    cy.get('#searchInput').type(text + '{enter}')
  ...
  function assertArticleTitleHasText(text) {
    cy.get('.mw-page-title-main').should('have.text', text)
  }
